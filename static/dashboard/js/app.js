          ...
          onApprove: function () {
            sendTextMessage().then((result) => {
              ...
              if (result.success === true) {
                ...
                ).innerHTML = `Message sent successfully. Id: ${result.data.Id}`;
              } else {
                document.getElementById(
                  "sendMessageContainer"
                ).innerHTML = `Problem sending message: ${result.error}`;
              }
            });
            return false;
          },
          ...
          onApprove: function () {
            deleteMessage().then((result) => {
              ...
              if (result.success === true) {
                ...
                ).innerHTML = `Message deleted successfully.`;
              } else {
                document.getElementById(
                  "deleteMessageContainer"
                ).innerHTML = `Problem deleting message: ${result.error}`;
              }
            });
            return false;
          },
      ...
      addInstance(fields)
        .then((result) => {
          if (result.success) {
            ...
            showError(
              "Failed to create instance: " + (result.error || "Unknown error")
            );
          }
        })
        .catch((error) => {
      ...
      .then((data) => {
        ...
        if (data.success && data.data && data.data.Users) {
          ...
          for (const [userJid, userData] of Object.entries(data.data.Users)) {
            ...
            userElement.innerHTML += `Status: ${
              userData.Status || "Not available"
            }<br>`;
            userElement.innerHTML += `Verified Name: ${
              userData.VerifiedName || "Not verified"
            }<br>`;
...
function populateInstances(instances) {
  ...
  instances.forEach((instance) => {
    const row = `
      <tr>
        ...
        <td>${instance.name}</td>
        <td><i class="${
          instance.connected ? "check green" : "times red"
        } icon"></i> <span class="status ${instance.connected}">${
      instance.connected ? "Yes" : "No"
    }</span></td>
        <td><i class="${
          instance.loggedIn ? "check green" : "times red"
        } icon"></i> <span class="status ${instance.loggedIn}">${
      instance.loggedIn ? "Yes" : "No"
    }</span></td>
        <td>
          <button class="ui primary button dashboard-button" onclick="openDashboard('${
            instance.id
          }', '${instance.token}')">
            <i class="external alternate icon"></i> Abrir
          </button>
          <button class="ui negative button dashboard-button" onclick="deleteInstance('${
            instance.id
          }')">
  ...
  `;
    ...
    const card = `
      <div class="ui fluid card hidden no-hover" id="instance-card-${
        instance.id
      }">
          <div class="content">
              <div class="ui ${
                instance.loggedIn ? "one" : "two"
              } column stackable grid">
                  ...
                  <div class="column">
                      <div class="header" style="font-size: 1.3em; margin-bottom: 0.5rem;">
                          ...
                          <div class="ui labels" style="margin-top: 0.5em;">
                              <div class="ui ${
                                instance.connected ? "green" : "red"
                              } horizontal label">
                                  <i class="${
                                    instance.connected ? "check" : "times"
                                  } icon"></i>
                                  ${
                                    instance.connected
                                      ? "Connected"
                              ...
                              <div class="ui ${
                                instance.loggedIn ? "green" : "red"
                              } horizontal label">
                                  <i class="${
                                    instance.loggedIn ? "check" : "times"
                                  } icon"></i>
                                  ${
                                    instance.loggedIn
                                      ? "Logged In"
                      ...
                      <div class="meta" style="margin-bottom: 1rem;">Instance ID: ${
                        instance.id
                      }</div>
                      ...
                      <div class="ui list">
                          ...
                          <div class="item">
                              ...
                              <div class="content" style="word-break: break-all;">${
                                instance.webhook || "Not configured"
                              }</div>
                          </div>
                          <div class="item">
                              <div class="header">Subscribed Events</div>
                              <div class="content">${
                                instance.events || "Not configured"
                              }</div>
                          </div>
                          <div class="item">
                          ...
                          <div class="item">
                              ...
                              <div class="content">${
                                instance.proxy_config.proxy_url ||
                                "Not configured"
                              }</div>
                          </div>
                          <div class="item">
                          ...
                          <div class="item">
                              ...
                              <div class="content">${
                                instance.s3_config.endpoint || "Not configured"
                              }</div>
                          </div>
                      </div>
                  </div>
            ...
            <div class="extra content">
              <button class="ui primary positive button dashboard-button ${
                instance.connected === true ? "hidden" : ""
              }" id="button-connect-${instance.id}" onclick="connect('${
      instance.token
    }')">Connect</button>
              <button class="ui primary negative button dashboard-button ${
                instance.connected === true ? "" : "hidden"
              }" id="button-logout-${instance.id}" onclick="logout('${
      instance.token
    }')">Logout</button>
              <button class="ui primary positive button dashboard-button ${
                instance.connected === true && instance.loggedIn === false
                  ? ""
                  : "hidden"
              } id="button-logout-${
      instance.id
    }" onclick="modalPairPhone()">Login with Pairing Code</button>
              </div>
        </div>
        `;
...
async function saveProxyConfig() {
  ...
  try {
    ...
    if (data.success) {
      ...
      showError(
        "Failed to save proxy configuration: " + (data.error || "Unknown error")
      );
    }
  } catch (error) {
