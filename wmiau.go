...
func (mycli *MyClient) myEventHandler(rawEvt interface{}) {
	...
	case *events.HistorySync:
		...
		// Extract basic information from history
		var messages []map[string]interface{}
		
		// Process conversations to extract basic information
		if len(evt.Data.Conversations) > 0 {
			for _, conv := range evt.Data.Conversations {
				// Create a basic message with conversation information
